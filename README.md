...
macOS and Linux remain part of the cross-platform contract, but their backends are contribution tracks that promote individual capabilities experimentally. Current macOS experimental coverage is limited to `read-only` with `network.disabled`. Current Linux experimental coverage is limited to `read-only` and `workspace-write` with `network.disabled`. A backend capability is promoted only when it passes the shared conformance suite and reports unsupported requests fail-closed.
