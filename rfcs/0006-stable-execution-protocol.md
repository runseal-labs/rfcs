...
- Client-to-server requests without an `id` are JSON-RPC notifications. RunSeal v1 defines server-to-client event notifications only; client-to-server notifications MUST be ignored without a response, including invalid notification objects, and MUST NOT mutate execution state.
