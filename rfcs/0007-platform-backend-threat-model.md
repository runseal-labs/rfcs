...
- Setup readiness diagnostics MAY expose coarse public state such as platform support, whether setup is required, broker readiness, and whether the current process can repair setup. Repairability MAY be true when the current process is elevated or when a previously installed setup broker can perform the repair without direct UAC interaction. They MUST NOT expose scheduled task names, helper paths, account names, SIDs, ACLs, WFP/firewall details, token attributes, or backend-private profile paths.
