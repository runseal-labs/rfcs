...
- The credential MUST be generated by the backend, scoped to the execution or active proxy handle, and hard to guess.
- The proxy MUST reject requests that omit or present the wrong local proxy credential.
- Audit events, errors, logs, and protocol results MUST redact the credential.
- The public protocol MUST NOT require clients to know the credential shape; it remains backend-private routing configuration.

Backends MAY use a stable OS-level network guard that allows only the managed proxy endpoint, provided direct egress remains blocked and proxy access is separately guarded. This avoids mutating firewall, WFP, Seatbelt, namespace, or equivalent enforcement state for every `disabled`/`proxy` execution.
